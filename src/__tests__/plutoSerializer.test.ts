...
z = 3`;
  ...
  describe('parsePlutoNotebook with demo.jl', () => {
    ...
    it('should have PlutoUI widgets', () => {
      const sliderCell = parsed.cells.find(c =>
        c.code.includes('@bind') && c.code.includes('Slider')
      );
      expect(sliderCell).toBeDefined();
    });

    it('should have plotting code', () => {
      const plotCells = parsed.cells.filter(c =>
        c.code.includes('plot(') || c.code.includes('scatter(') || c.code.includes('heatmap(')
      );
  ...
  describe('Round-trip with demo.jl', () => {
    it('should parse and serialize demo notebook without data loss', async () => {
      ...
      expect(reParsed.cells[reParsed.cells.length - 1].code).toBe(originalLastCell);
    });
    ...
    it('should preserve specific cell content', async () => {
      ...
      // Find specific cells
      const xCell = parsed.cells.find(c => c.code.trim() === 'x = 5');
      const titleCell = parsed.cells.find(c => c.code.includes('Pluto Notebook Demo'));
      ...
      const titleCellAfter = reParsed.cells.find(c => c.code.includes('Pluto Notebook Demo'));
